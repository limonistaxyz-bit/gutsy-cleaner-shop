...
const initialBatch={remaining:12,capacity:12,pickupLabel:"Saturday, October 3, 2026",cutoffLabel:"Thursday, October 1, 6:00 PM",isOpen:false};
...
export default function Home(){
 ...
 useEffect(()=>{fetch("/api/inventory",{cache:"no-store"}).then(r=>{if(!r.ok)throw new Error("Unavailable");return r.json();}).then(x=>setBatch(x as typeof initialBatch)).catch(()=>{});},[]);
 ...
 const checkout=async(e:FormEvent)=>{e.preventDefault();setBusy(true);setError("");try{const fingerprint=JSON.stringify({...form,cart});if(checkoutAttempt.current?.fingerprint!==fingerprint)checkoutAttempt.current={fingerprint,id:crypto.randomUUID()};const response=await fetch("/api/checkout",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({...form,cart,checkoutId:checkoutAttempt.current.id})});const data=await response.json() as {error?:string;url:string};if(!response.ok)throw new Error(data.error||"Checkout could not start.");location.href=data.url;}catch(e){setError(e instanceof Error?e.message:"Checkout could not start.");setBusy(false);}};
 return <main>
