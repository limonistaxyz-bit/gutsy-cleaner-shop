...
export async function POST(request:Request){
 const user=await getChatGPTUser(); const admin=(env as unknown as Record<string,string|undefined>).ADMIN_EMAIL;
 if(!user||!admin||user.email.toLowerCase()!==admin.toLowerCase())return Response.json({error:"Owner access required."},{status:403});
 ...
 if(pickup.getDay()!==6)return Response.json({error:"Pickup must be a Saturday."},{status:400});
 const capacity=Math.floor(Number(body.capacity)); if(capacity<1||capacity>100)return Response.json({error:"Batch limit must be between 1 and 100."},{status:400});
 ...
 const database=db();
 const existing=await database.prepare("SELECT reserved FROM batches WHERE id = ?").bind(pickupDate).first<{reserved:number}>();
